...
This guide covers how the repo wires Microsoft Graph change notifications into an Event Grid partner topic, routes them to a Storage Queue, and processes them with the `process-user-changes` queue-triggered function. It demonstrates a complete showcase of Microsoft Graph integration with Azure Integration Services, including change notification subscriptions for user, group, and permission changes, Event Grid routing, and serverless processing with Azure Functions (PowerShell runtime). It also explains the two deployment options (Function-managed vs. Bicep-managed event subscriptions) and how the partner configuration is shared across function apps in the same resource group.

## Architecture at a glance

1. **Graph subscription** publishes user/group/permission change notifications to Event Grid using the `EventGrid:` notification URL scheme.
2. **Event Grid partner topic** is created automatically by Microsoft Graph when the subscription is created.
3. **Partner topic activation** enables routing of change notifications.
4. **Event subscription** on the partner topic delivers CloudEvents to a Storage Queue using a managed identity.
5. **Queue-triggered function** (`process-user-changes`) processes Graph change notifications and (optionally) renews subscriptions on lifecycle events.
6. **Scheduled queries** can also capture periodic snapshots of Graph data and store them in blob storage for historical analysis.
...
- A **Storage Queue** (`graph-user-changes-q`) for Graph change notifications.
- **Role assignments**:
...
- `GRAPH_USER_CHANGES_QUEUE_NAME`: Queue name (default `graph-user-changes-q`).
...
When creating a Graph subscription for user/group/permission changes:

- Use `resource = "users"` (or "groups" for group changes) and `changeType = "created,updated,deleted"`.
...
- `expirationDateTime` for `users` and `groups` resources has a **maximum of 3 days**, so set a shorter window (the code uses 2 days) and renew regularly.
